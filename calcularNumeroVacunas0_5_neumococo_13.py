...
df_fre['frecuencia_nacimiento_primera'] = calcular_frecuencia(
    ...
    df_fre['neumococo_conjugado_13_valente_primera']
)

df_fre['frecuencia_primera_segunda'] = calcular_frecuencia(
    df_fre['neumococo_conjugado_13_valente_primera'], 
    df_fre['neumococo_conjugado_13_valente_segunda']
)

df_fre['frecuencia_segunda_tercera'] = calcular_frecuencia(
    df_fre['neumococo_conjugado_13_valente_segunda'], 
    df_fre['neumococo_conjugado_13_valente_tercera']
)
...
def calcular_frecuencia_general(row):
    frecuencias = [row['frecuencia_nacimiento_primera'], 
                   ...
                   row['frecuencia_segunda_tercera']]
...
print(df_fre[['tipoidentificacion', 'frecuencia_nacimiento_primera', 'frecuencia_primera_segunda', 
              'frecuencia_segunda_tercera', 'frecuencia_general']].head())

###############################################################################################
#                        Generar CSV SOLO POR AHORA CAMBIAR A TABLA                           #
###############################################################################################

df_fre.to_csv('csv/data_mart_frecuencia_neumococo_13.csv', sep = '|', index = False, encoding = 'latin1')

df_mart = pd.read_csv("csv/data_mart_frecuencia_neumococo_13.csv", sep = '|', encoding = 'latin1')
